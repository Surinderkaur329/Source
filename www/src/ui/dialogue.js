...
LetiFramework.Ui.dialogue.prototype = {
	...
	preload: function() {
		...
		if (readModeBtConfig.type == "image") {
			...
			LetiFramework.Utils.loadImage(readModeBtImg.auto);
			LetiFramework.Utils.loadImage(readModeBtImg.manual);
		}
	...
	create: function() {
		...
		this.homeBt = this.game.add.button(optionsBtPosition.x, this.currentGamePageData.show_menu_items ?
			this.screenConfig.options_button.children.home_button.position.y : optionsBtPosition.y,
		...
		for (var i = 0; i < this.dialogues.length; i++) {
			var dialogue = this.dialogues[i];
			var soundKey = this.getSoundPath(dialogue.sound);
			if (LetiFramework.App.isPhoneGap()) {
				this.dialogueSounds.push(dialogue.sound.length > 0 && LetiFramework.SoundManager.soundOn ?
					LetiFramework.SoundManager.getSound(soundKey) : null);
			} else {
				this.dialogueSounds.push(dialogue.sound.length > 0 && LetiFramework.SoundManager.soundOn ?
					this.game.add.audio(soundKey) : null);
			}
		}
		...
		if (this.currentGamePageData.text.length > 0) {
			...
			if (this.currentGamePageData.custom_font) {
				...
				this.narrText = this.game.add.bitmapText(this.narrTextBg.x + this.narrTextBg.width / 2, this.narrTextBg.y + this.narrTextBg.height / 2,
					LetiFramework.Utils.getAssetFontPath(cf), LetiFramework.Utils.wrapText(this.currentGamePageData.text, cf), cf.font_size);
			} else {
		...
		if (readModeBtConfig.type == "image") {
			...
			this.readModeBt = this.game.add.sprite(readModeBtConfig.position.x, readModeBtConfig.position.y,
				LetiFramework.Utils.getImagePath(readModeImg));
	...
	readModeHandler: function() {
		...
		if (readModeBtConfig.type == "button") {
			...
			var readModeBtImg = readModeBtConfig.image;
			this.readModeBt.loadTexture(LetiFramework.Utils.getImagePath(this.currentUser.dialogueReadMode == 0 ?
	...
	addHud: function() {
		if (this.hud.visible && !this.currentGamePageData.hide_hud) {
			...
			if (this.hud.text_custom_font) {
				...
				scoreText = this.game.add.bitmapText(scoreLabel.width + 10, 0,
					LetiFramework.Utils.getAssetFontPath(cf), "" + this.currentUser.points, cf.font_size);
			} else {
	...
	nextDialogue: function() {
		...
		if (this.dialogueIdx < this.dialogues.length) {
            ...
            if(dialogue.username){
                dialogue.username = JSON.parse(window.localStorage.getItem("session"));
            }
            if(dialogue.username === undefined){
                dialogue.username = '';
			}
            if(dialogue.text2 === undefined){
                dialogue.text2 = '';
            }
			...
			if (text_pos) {
				...
				if (dialogue.custom_font) {
					...
					this.speechText = this.game.add.bitmapText(text_pos.x, text_pos.y,
						LetiFramework.Utils.getAssetFontPath(cf), LetiFramework.Utils.wrapText(dialogue.text+""+dialogue.username+""+dialogue.text2, cf), cf.font_size);
				} else {
					this.speechText = this.game.add.text(text_pos.x, text_pos.y, dialogue.text+""+dialogue.username+""+dialogue.text2, text_style);
				}
			} else {
				if (text_style.width) {
					...
					this.speechText = this.game.add.text(0, 0, dialogue.text+""+dialogue.username+""+dialogue.text2, text_style);
					...
					if (dialogue.bubble.padding.constructor === Array) {
						...
						this.speechText = this.game.add.text(this.speechBubble.x + this.speechBubble.width / 2,
							this.speechBubble.y + pT, dialogue.text+""+dialogue.username+""+dialogue.text2, text_style);
						...
						this.speechText = this.game.add.text(this.speechBubble.x + this.speechBubble.width / 2,
							this.speechBubble.y + dialogue.bubble.padding, dialogue.text+""+dialogue.username+""+dialogue.text2, text_style);
						this.speechText.anchor.set(0.5, 0);
					}
				}
			}
	...
	getBubblePicPath: function(name) {
		return LetiFramework.Utils.removeDotPath('assets/stories/' + this.currentGame.storyId + '/episodes/' +
			this.currentGame.id + '/content/bubbles/' + name);
	},
	getCharacterPicPath: function(name) {
		return LetiFramework.Utils.removeDotPath('assets/stories/' + this.currentGame.storyId + '/episodes/' +
			this.currentGame.id + '/content/characters/' + name);
	},
