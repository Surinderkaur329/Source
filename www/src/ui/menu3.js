...
LetiFramework.Ui.menu3 = function(game) {};

LetiFramework.Ui.menu3.prototype = {
    init: function() {
        LetiFramework.Analytics.trackPage("Menu3");
    ...
    create: function() {
        ...
        for (var i in this.buttons) {
            ...
            img.events.onInputDown.add(function(src) {
                ...
                if(this.button.name === "GAME"){
                    LetiFramework.GameController.bootScreen = this.button.next_screen;
                }else{
                    window.localStorage.setItem('game_url',JSON.stringify(window.location.href));
                    window.localStorage.setItem('game_url_type','main');
                    var url = this.button.game_url;
                    var url2 = this.button.web_game_url;

                    var orientation = this.button.orientation;
                    if (orientation && orientation === "portrait" && LetiFramework.App.isPhoneGap()) {
                        screen.orientation.lock('portrait');
                    }
                    if (url.length > 0) {
                        LetiFramework.GameController.saveInstance();
                        if(LetiFramework.App.isPhoneGap()){
                    ...
                    LetiFramework.GameController.bootScreen = this.button.url;
                }
