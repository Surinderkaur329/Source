...
LetiFramework.Ui.choice.prototype = {
    ...
    create: function() {
        ...
        if (this.currentGamePageData.custom_font) {
            ...
            this.titleText = this.game.add.bitmapText(this.titleTextBg.x + this.titleTextBg.width / 2,
                this.titleTextBg.y + this.titleTextBg.height / 2, LetiFramework.Utils.getAssetFontPath(cf),
            ...
            if (choice.text.length > 0) {
                ...
                if (choice.custom_font) {
                    ...
                    this.choiceText = this.game.add.bitmapText(
                        ...
                        LetiFramework.Utils.getAssetFontPath(cf), LetiFramework.Utils.wrapText(choice.text, cf), cf.font_size);
                } else {
    ...
    awardChoicePoints: function(selection, selectedChoice) {
        ...
        if (this.choices[selectedChoice].player_attributes) {
            var pas = this.choices[selectedChoice].player_attributes;
            for (var k in pas) {
                this.currentUser.player_attributes[k] += pas[k];
            }
            LetiFramework.Db.update("users", this.currentUser);
