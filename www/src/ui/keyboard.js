...
LetiFramework.Ui.keyboard.prototype = {
	init: function() {
		this.inputText = "";
		this.prevScreen = stores.get("prevScreen");
		this.maxInputLength = stores.get("maxLength");
		this.isNumber = stores.get("number");
		stores.remove("number");
		stores.remove("maxLength");
		stores.remove("prevScreen");
	},
	preload: function() {
		        	...
		        	fxns[i], style[0], style[1], function() { 
		        		switch(this.getText()) {
	        				...
	        				case 'Ok':
	        					...
	        					stores.set("userInput", $.trim(input));	        					
	        					LetiFramework.Renderer.render(prevScreen);
		        				break;
	        				default:	        					
		        				break;
