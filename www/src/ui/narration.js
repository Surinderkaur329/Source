...
LetiFramework.Ui.narration.prototype = {
	...
	preload: function() {
		...
		LetiFramework.Utils.loadImage(this.screenConfig.narration_side_panel.read_button.image);
		LetiFramework.Utils.loadImage(this.screenConfig.narration_side_panel.narr_next_button.image);
		if (this.narrationSidePanelImage && this.narrationSidePanelImage.length > 0) {
			this.loadImageFile(this.narrationSidePanelImage);
		} else {
			LetiFramework.Utils.loadImage(this.screenConfig.narration_side_panel.image);
		}
		...
		if (!LetiFramework.App.isPhoneGap()) {
			...
			this.loadAudio(this.currentGamePageData.narration.audio);
	...
	create: function() {
		...
		if (this.narrations.length > 0) {
			...
			this.readBt = this.game.add.button(readBtPosition.x, readBtPosition.y, 
				LetiFramework.Utils.getImagePath(this.screenConfig.narration_side_panel.read_button.image), function() {
			...
			if (this.currentGamePageData.custom_font) {
				...
				this.narrationText = this.game.add.bitmapText(-halfNarrW, this.game.world.centerY, LetiFramework.Utils.getAssetFontPath(cf),
					LetiFramework.Utils.wrapText(this.narrations[this.narrationIdx], cf), cf.font_size);
			} else {
			...
			if (this.narrations.length > 1) {
				...
				this.narrNextBt = this.game.add.button(-halfNarrW, narrNextBtPosition.y, 
					LetiFramework.Utils.getImagePath(this.screenConfig.narration_side_panel.narr_next_button.image), function() {
		...
		if (LetiFramework.App.isPhoneGap()) {
			...
			this.narrationSound = this.game.add.audio(this.getSoundPath(this.currentGamePageData.narration.audio));
		}
