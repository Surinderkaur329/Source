...
LetiFramework.Ui.register.prototype = {
    ...
    preload: function() {
        ...
        for (var i in this.otherRegisterFields) {
            ...
            if (field.drop_down == true) {
                LetiFramework.Utils.loadImage(field.drop_down_bg);
            }
        }
    },
    create: function() {
        ...
        if (inputType == "nickname") {
            ...
            if (input) stores.set("nickname", input);
        ...
        for (var i = 0; i < this.otherRegisterFields.length; i++) {
            ...
            var regFieldVal = stores.get(regFieldName);
            ...
            if (inputType == regFieldName) {
                ...
                if (input) stores.set(regFieldName, input);
                else stores.remove(regFieldName);
            }
        ...
        for (var i = 0; i < this.otherRegisterFields.length; i++) {
            ...
            if (regField.drop_down == true) {
                var dropDownBg = this.game.add.sprite(regField.position.x, regField.position.y, LetiFramework.Utils.getImagePath(regField.drop_down_bg));
        ...
        if (regBtConfig.type == "image") {
            var registerBt = this.game.add.sprite(regBtConfig.position.x, regBtConfig.position.y, LetiFramework.Utils.getImagePath(regBtConfig.image));
        ...
        if (cancelBtConfig.type == "image") {
            var cancelBt = this.game.add.sprite(cancelBtConfig.position.x, cancelBtConfig.position.y, LetiFramework.Utils.getImagePath(cancelBtConfig.image));
    ...
    nicknameHandler: function() {
        ...
        stores.set("prevScreen", "Register");
    ...
    registerHandler: function() {
        ...
        if (!nickname || $.trim(nickname).length == 0) {
            ...
            for (var i = 0; i < this.otherRegisterFields.length; i++) {
                ...
                } else if (regField.confirm && stores.get(regField.confirm) != fieldValue) {
                    passedValidation = false;
            ...
            if (passedValidation) {
                ...
                if (users.length > 0) {
                    ...
                    if (LetiFramework.App.isPhoneGap() || LetiFramework.GameController.bootConfig.web_version) {
                        var self = this;
                        // register on server
    ...
    registerSuccessHandler: function(user) {
        ...
        stores.set("session", user.nickname);
        this.clearFields();
    ...
    listenSwipe: function(callback) {
        ...
        this.game.input.onUp.add(function(pointer) {
            ...
            if (eventDuration > minimum.duration && actionSwipe) {
                ...
                if (this.dd) {
                    if (this.dd.item) {
                        stores.set(this.dd.model.name, this.dd.item);
